...
obj_names = ['hero','door','moster','money','boss','option','fanpai']
action_cache = ""
        ...
        if frame % fs == 0:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    if "hero" not in cls_object and "option" not in cls_object:
                        move1(direct="RIGHT")

                    #什么都没有，就往右边走
                    if "door" not in cls_object and "moster" not in cls_object and "boss" not in cls_object \
                             and "money" not in cls_object and "option" not in cls_object:
                         move1(direct="RIGHT")
